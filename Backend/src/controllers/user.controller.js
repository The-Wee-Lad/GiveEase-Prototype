  ...
  try {
    ...
    try {
      ...
      if (error.name == "TokenExpiredError") {
        res
          .status(401)
          .json(new ApiResponse(401, {}, "Expired Refresh Token", 719));
    ...
    if (!user) {
      res
        .status(401)
        .json(new ApiResponse(401, {}, "Invalid Refresh Token", 719));
    ...
    try {
      ...
      res
        .status(500)
        .json(new ApiResponse(401, {}, "Couldn't Generate Tokens", 720));
    ...
    throw new Error(
      `couldn't refresh Access Token [Refresh Access Token] ${error}`
    );
  }
});
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!usernameOrEmail || !password) {
    return res
      .status(400)
      .json(new ApiResponse(400, {}, "All Fields Required", 700));
  }
  ...
  if (!user) {
    return res
      .status(401)
      .json(new ApiResponse(401, {}, "Invalid Credentials", 707));
  }
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (await User.findOne({ username: username.toLowerCase() })) {
    return res
      .status(409)
      .json(new ApiResponse(400, {}, "Username Already in Use", 701));
  }
  if (await User.findOne({ email: email })) {
    return res
      .status(409)
      .json(new ApiResponse(400, {}, "Email Already in use", 701));
  }
  ...
  if (!user) {
    res
      .status(500)
      .json(
        new ApiResponse(500, {}, "User Registration failed Server Error", 703)
      );
  }
  /*res
    .status(200)
    ...
    //here tokens are not being generated.....
...
const getCurrentUser = asyncHandler(async (req, res) => {
  if (!req?.user) {
    res
      .status(500)
      .json(new ApiResponse(500, {}, "Couldn't Fetch Cyrrent User", 702));
    throw new Error(
      "Verification failed yet the data passed forward to getcurrentUser"
    );
  }
